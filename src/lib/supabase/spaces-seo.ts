...
export async function getCitiesWithStats(): Promise<CityStats[]> {
  ...
  for (const row of data ?? []) {
    ...
    if (!cityMap.has(properCity))
      cityMap.set(properCity, { count: 0, neighborhoods: new Map() });
    const entry = cityMap.get(properCity)!;
    entry.count++;
    if (row.neighborhood) {
      entry.neighborhoods.set(
        row.neighborhood,
        (entry.neighborhoods.get(row.neighborhood) ?? 0) + 1,
      );
    }
  }
...
function buildCityMap(rows: any[]) {
  ...
  for (const row of rows) {
    ...
    if (!existing) {
      ...
      existing.toUpperCase() === existing &&
      row.city.toUpperCase() !== row.city
    ) {
...
export async function getDistinctCityTypePairs(): Promise<
...
export async function getSpacesByCity(
  city: string,
  type?: string,
): Promise<Space[]> {
  const supabase = createSeoClient();
...
export async function getAllActiveSpacesSummary(): Promise<
...
> {
  ...
  return (data ?? []).map((r) => ({
    ...
    city: r.city
      ? (casingMap.get((r.city as string).toLowerCase()) ?? null)
      : null,
