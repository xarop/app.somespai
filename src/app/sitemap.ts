...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  for (const slug of slugs) {
    for (const locale of LOCALES) {
      entries.push({ url: u(`/espai/${slug}`, locale), changeFrequency: 'weekly', priority: 0.8 });
    }
  }
  ...
  for (const city of cities) {
    ...
    for (const locale of LOCALES) {
      entries.push({ url: u(`/${citySlug}`, locale), changeFrequency: 'daily', priority: 0.9 });
    }
  }
  ...
  for (const { city, type } of pairs) {
    ...
    for (const locale of LOCALES) {
      ...
      entries.push({ url: u(`/${citySlug}/${typeSlug}`, locale), changeFrequency: 'daily', priority: 0.85 });
    }
  }
