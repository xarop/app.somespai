...
const STREET_PREFIX_RE =
  /^(carrer|avinguda|passeig|plaça|ronda|via|rambla|travessera|gran via|calle|avenida|paseo|plaza|cami|camí|pg\.|av\.|c\.)\s+(de\s+les?|dels?|d'|de\s+l'|de\s+)?/i;
...
export function PublishForm({ isLoggedIn = true }: PublishFormProps) {
  ...
  const [autofillState, setAutofillState] = useState<
    "idle" | "locating" | "geocoding" | "done" | "error"
  >("idle");
      ...
      async (position) => {
        ...
        try {
          ...
          if (titleRef.current && !titleRef.current.value) {
            ...
            const streetPart = [shortRoad, data.houseNumber]
              .filter(Boolean)
              .join(" ");
            const location = [streetPart, data.neighborhood || data.city]
              .filter(Boolean)
              .join(", ");
            titleLocationRef.current = location;
            const typeLabel = selectedType ? tSpaceType(selectedType) : null;
            titleRef.current.value = typeLabel
              ? `${typeLabel} ${location}`
              : location;
  ...
  function handleTypeChange(type: SpaceType) {
    ...
    if (titleAutofilled && titleRef.current) {
      ...
      titleRef.current.value = location
        ? `${tSpaceType(type)} ${location}`
        : tSpaceType(type);
    }
  }
  ...
  if (state === "SUCCESS_GUEST") {
    return (
      <div className="publish-success">
        ...
        <p>
          Com que no havies iniciat sessió, l'espai està{" "}
          <strong>pendent de validació</strong>.
          <br />
          <br />
          Contactarem amb tu a l'adreça de correu que has indicat per verificar
          la teva identitat i publicar l'espai definitivament.
        </p>
      </div>
    );
  }
  ...
  return (
    <form action={formAction} className="publish-form">
      ...
      {!isLoggedIn && (
        ...
        >
          <div>
            <strong>No has iniciat sessió.</strong>
            <br />
            Inicia sessió o crea un compte gratuït per publicar i gestionar
            directament els teus espais. O si ho prefereixes, pots publicar
            l'espai com a invitat deixant el teu correu electrònic a
            continuació.
          </div>
        ...
        <fieldset className="fieldset">
          ...
          <label className="field">
            ...
            <p className="field__help">
              Necessitem el teu email per crear l'usuari i vincular-hi aquest
              espai.
            </p>
          </label>
        </fieldset>
      ...
      <fieldset className="fieldset">
        ...
        <div className="geo-autofill" data-status={autofillState}>
          <button
            ...
            disabled={
              autofillState === "locating" || autofillState === "geocoding"
            }
            className="geo-autofill__button"
          >
            <span className="geo-autofill__icon" aria-hidden="true">
              📍
            </span>
          ...
          {autofillState === "done" && (
            <p className="geo-autofill__hint" role="status">
              {t("autofillSuccess")}
            </p>
          )}
          {autofillState === "error" && (
            <p className="geo-autofill__hint" role="alert">
              {t("autofillError")}
            </p>
          )}
        </div>
        ...
        {geoState === "loading" && (
          <p className="field__hint">{t("geolocating")}</p>
        )}
        {geoState === "error" && (
          <p className="field__hint field__hint--error">{t("geoError")}</p>
        )}
                ...
                <div className="checkbox-grid">
                  {visible.map((id) => (
                    <label key={id} className="checkbox-item">
                      ...
                      <span>
                        {tAmenity(id as Parameters<typeof tAmenity>[0])}
                      </span>
                    </label>
                  ))}
                </div>
