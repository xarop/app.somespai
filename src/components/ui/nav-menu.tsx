...
export function NavMenu() {
  ...
  useEffect(() => {
    ...
    const checkAdmin = async (u: User | null) => {
      ...
      if (u.email === process.env.NEXT_PUBLIC_ADMIN_EMAIL) { setIsAdmin(true); return; }
  ...
  async function handleSignOut() {
    ...
    try { window.localStorage.removeItem("dev_admin_bypass"); } catch { /* ignore */ }
    document.cookie = "dev_admin_bypass=; path=/; max-age=0";
    const supabase = createClient();
      ...
      >
        ...
        <div className="dropmenu__body">
          {user ? (
            ...
            <button type="button" role="menuitem" onClick={() => { setMenuOpen(false); setAuthOpen(true); }}>
              <Icon name="user" size={16} />
              {t("nav.signIn")}
            </button>
          )}
          ...
          <a href="https://somespai.net" role="menuitem" target="_blank" rel="noopener noreferrer" onClick={() => setMenuOpen(false)}>
        ...
        <div className="dropmenu__footer">
          ...
          {LOCALES.map((l) => (
            <button key={l.code} type="button" role="menuitemradio" aria-checked={locale === l.code} onClick={() => switchLocale(l.code)}>
