...
export function MapView({
  ...
  useEffect(() => {
    ...
    const style: string =
      process.env.NEXT_PUBLIC_MAP_TILES_URL || CARTO_POSITRON;
  ...
  useEffect(() => {
    ...
    const syncMarkers = () => {
      ...
      const nextIds = new Set(
        clusters.map((c) =>
          c.properties.cluster ? `cluster-${c.id}` : c.properties.id,
        ),
      );
  ...
  useEffect(() => {
    ...
    const fly = () => {
      if (spaces.length === 1) {
        map.flyTo({
          center: [spaces[0].lng, spaces[0].lat],
          zoom: 15,
          duration: 800,
        });
        return;
  ...
  useEffect(() => {
    for (const [id, marker] of markersRef.current) {
      marker
        ...
        .setAttribute(
          "data-hover",
          String(id === hoveredSpaceId && hoveredSpaceId !== null),
        );
    }
  }, [hoveredSpaceId]);
